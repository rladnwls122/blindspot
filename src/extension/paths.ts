...
export function relativeKey(root: string, fsPath: string): string | null {
  ...
  // A different drive on Windows relativises to an absolute path, which `..`
  // alone does not catch.
  if (!normalized || path.isAbsolute(normalized) || /^[a-zA-Z]:/.test(normalized)) return null;
  // Only a leading `..` segment means "outside"; a file really named `..cache`
  // is inside, and `startsWith('..')` used to throw it away.
  if (normalized.split('/')[0] === '..') return null;
  return normalized;
}
