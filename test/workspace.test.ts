...
import { relativeKey } from '../src/extension/paths';
...
describe('relativeKey', () => {
  ...
  test('a file inside the root becomes a forward-slash relative key', () => {
    assert.equal(relativeKey(root, inside('src', 'a.ts')), 'src/a.ts');
  });

  test('a file outside the root is not ours', () => {
    assert.equal(relativeKey(root, path.join(path.dirname(root), 'other', 'a.ts')), null);
    assert.equal(relativeKey(root, root), null);
  });
