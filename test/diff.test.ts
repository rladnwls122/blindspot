...
describe('unquote', () => {
  test('decodes octal UTF-8 bytes, one character across several escapes', () => {
    assert.equal(unquote('"\\355\\225\\234.ts"'), '한.ts');
  });
  ...
  test('handles the C escapes git uses for quotes, backslashes and tabs', () => {
    ...
    assert.equal(unquote('"q\\"uote 한.ts"'), 'q"uote 한.ts');
    assert.equal(unquote('"back\\\\slash.ts"'), 'back\\slash.ts');
    assert.equal(unquote('"tab\\there.ts"'), 'tab\there.ts');
  });

  test('leaves a bare path alone, spaces included', () => {
    assert.equal(unquote('src/with space.ts'), 'src/with space.ts');
