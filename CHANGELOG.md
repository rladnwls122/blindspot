...
- `npm test` passed a directory to `node --test`, which Node 22 rejects. A
  small runner (`scripts/run-tests.mjs`) now hands the runner an explicit file
  list, which every supported Node version and shell agrees on.
