...
패널은 같은 이야기를 파일 단위로 합니다. 파일의 모든 줄이 나오고, 점수를 채운
줄은 흐리게, 못 채운 줄은 여백에 표시됩니다 — 리뷰했다고 생각한 diff에 안 읽은
부분이 그대로 남아 있는 그림입니다.

![Blindspot 리포트 패널의 파일 한 개 — 읽은 것으로 인정된 줄은 흐리게, 안 읽은 줄은 여백에 표시](media/evidence.png)
...
[Open VSX](https://open-vsx.org/extension/rladnwls122/blindspot)에서 설치할 수 있습니다 —
VSCodium, Cursor, Windsurf, Gitpod가 확장을 받아 가는 곳입니다. VS Code 본체에서는
`.vsix`를 만들어 설치합니다.
...
줍니다. 멀티루트 워크스페이스의 폴더는 각각 따로 추적되고,
상태 표시줄과 사이드바는 지금 읽고 있는 폴더를 따라갑니다.
